...
## 트러블 슈팅
프로젝트를 진행하며 마주친 주요 문제들과 해결 과정입니다.

* **이메일 중복 가입 시 DB 제약조건 위반 (500 에러) 방지**
  * **문제**: 동일한 이메일로 가입 시 DB 제약조건 위반으로 인해 서버 에러가 발생.
  * **해결**: 회원가입 컨트롤러에 이메일 중복 검증 로직을 추가하고 예외를 캐치하여 글로벌 에러(Object Error) 메시지로 처리, 사용자에게 명확한 피드백 제공.
* **검색 후 페이지 이동 시 검색 조건 유실 문제 해결**
  * **문제**: 검색 후 다음 페이지로 이동 시 검색 키워드 파라미터가 누락되어 조건이 해제되는 현상.
  * **해결**: 타임리프 페이징 처리 링크(`th:href`)에 현재 검색 조건(`type`, `keyword`) 파라미터가 항상 포함되도록 수정하여 검색 상태 유지 기능 개선.


<br>

## 향후 보완할 점
* **소셜 로그인 (OAuth 2.0)**: Google, Naver 등을 통한 간편 로그인 기능 도입
* **비밀번호 재설정 기능**: 이메일 인증 등을 통한 안전한 비밀번호 재설정 기능 추가
* **마이페이지 기능**: 내 정보 수정 및 작성한 글/댓글을 관리할 수 있는 공간 마련
* **파일 첨부 기능**: 게시글 작성 시 이미지 및 문서 첨부 기능 추가

<br>

## 후기

비록 간단한 형태의 게시판 프로젝트이지만, 그동안 학습한 내용을 복습하고 스스로 한 단계 성장할 수 있었던 소중한 경험이었습니다. 자바와 스프링 프레임워크의 깊고 넓은 생태계를 꾸준히 학습해 오면서 제 이해도를 객관적으로 점검할 시점이 필요했고, 이번 프로젝트를 직접 처음부터 구현해 보며 그동안 배운 내용들을 단단하게 다질 수 있었습니다. 개발 과정에서 막연하게 다가왔던 개념들을 더 깊이 이해하고 잘못된 지식을 바로잡으며, '학습한 내용을 실제 코드로 구현해 내는 것의 중요성'을 깨달았습니다.

특히 제3자의 시각에서 제 코드를 테스트하고 피드백을 받는 과정을 통해, 개발자 혼자서는 놓치기 쉬운 예외 상황들을 발견할 수 있었습니다. 이를 바탕으로 코드를 수정하고 프로젝트의 완성도를 높여가며 테스트와 피드백 수용의 중요성을 몸소 느꼈습니다. 앞으로도 스프링 MVC의 동작 원리를 더 깊이 파고들고 JPA 등 새로운 기술들을 꾸준히 학습하며, 이 프로젝트를 지속적으로 리팩토링하고 발전시켜 나갈 계획입니다.
### 끝까지 읽어주셔서 감사합니다.
